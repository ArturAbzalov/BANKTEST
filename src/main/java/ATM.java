    ...
    public static void main(String[] args) {
        while (true) {
            try {
                ...
                clientSocket = new Socket("localhost", 3347);
                in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
                out = new BufferedWriter(new OutputStreamWriter(clientSocket.getOutputStream()));
                connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/atm",
                        "ATM", "123");
                                ...
                                if (Integer.parseInt(input) == 3) {
                                    while (true) {
                                        try {
                                            ...
                                            if (checkPatternNumber(number) && Long.parseLong(number) > 0
                                                    && Long.parseLong(sum) > 0) {
                                                out.write(3 + "\n");
                                                out.flush();
                                                out.write(number + "\n");
                                                out.flush();
                                                ...
                                                if (serverAnswer == 1) {
                                                    System.out.println("Операция завершена, ваш баланс "
                                                            + Integer.parseInt(in.readLine()));
                                                    break;
                                                }
                                                if (serverAnswer == 2) {
                                        ...
                                        try {
                                            ...
                                            if (total % 100 == 0) {
                                                ...
                                                System.out.println("Операция завершена успешно," +
                                                        "Ваш баланс составляет: " + Integer.parseInt(in.readLine()));
                                                break;
                                            } else {
                                                System.out.println("Некорректная сумма, введите сумму кратную 100");
                                            }
                ...
                System.out.println("Неудалось подключиться к базе данных ");
            } finally {
                try {
                    statement.close();
                    clientSocket.close();
